...
10. `experiments/kg_ontology_v2_coordination/claim_map.json`
11. The README and result artifact for every experiment being compared.
...
- Before saying that KG works, ontology fails, Ontology v2 lacks incremental
  ...
  `experiments/kg_ontology_v2_coordination/claim_map.json` and cite the exact
  artifacts supporting the statement.
- The current bounded statement is: Ontology v2 shows clear incremental value
  over KG-only on the fixed redacted coordination hard challenge; transfer of
  that full coordination-frame advantage to the current operator-provided
  full-PST pipeline remains unverified.
