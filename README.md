...
<!-- Future agents: read AGENTS.md first, then use docs/implementation-task-breakdown.md as the shared checklist. Treat the methodology-first SPEC.md as canonical; rewrite affected sections instead of appending contradictory overrides, and do not create parallel replacement files unless the specification is changed first. -->

formowl is a source-preserving, graph-governed knowledge system. It turns
heterogeneous evidence into governed, reusable knowledge without making any
single source format, department, or output surface the product core.
...
Email, procurement, finance, project systems, documents, media, conversations,
and operational systems all use this same methodology. Source-specific
extractors and scoped domain packs adapt the method; they do not create
parallel knowledge pipelines. Wiki generation is one projection surface among
answers, reports, dashboards, review queues, and action proposals.
...
Project MCP and Wiki MCP remain decoupled compatibility services. Project MCP
retrieves project execution context from systems such as OpenProject. Wiki MCP
generates and manages markdown/wiki knowledge artifacts. Their JSON-line and
hand-built JSON-RPC/stdio entrypoints are for tests and local compatibility;
they are not alternate ChatGPT identity or connection paths.
...
Implementation layers add source adapters, asset ingestion, observation
extraction, candidate assertion review, canonical graph commits, user/task
effective views, and governed projections behind the same method.
...
- Sources never directly become canonical knowledge or final business output.
- Extractors produce observations and candidate knowledge, never canonical
  truth.
...
- Candidate business objects, assertions, atoms, relations, and frames are
  reviewed before canonical graph commit.
...
- Different users and tasks can assemble different effective graph views from
  the same governed canonical graph.
- Wiki revisions, reports, dashboards, answers, and action proposals are
  governed output artifacts with citations, evidence snapshots, graph lineage,
  and review state.

## Specifications

- `SPEC.md` - the canonical methodology-first product and architecture
  specification, including domain portability, graph governance, effective
  views, and projection boundaries.
