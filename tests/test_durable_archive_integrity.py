    ...
    def test_manifest_matches_every_lossless_snapshot(self) -> None:
        ...
        for archive_date, expected_file_count in SNAPSHOT_FILE_COUNTS.items():
            ...
            for item in manifest["files"]:
                self.assertNotIn(item["archive"], archived_paths)
                archived_paths.add(item["archive"])
                archive_path = (ROOT / item["archive"]).resolve()
                try:
                    archive_path.relative_to(snapshot_root.resolve())
                except ValueError:
                    self.fail(f"archive path escaped snapshot root: {archive_path}")
