...
class OAuthBridgeServiceTests(unittest.TestCase):
        ...
        operations = (
            ...
            (
                ...
                lambda: fixture.bridge.revoke_token_session(
                    "oauthsid_discovery",
                    revoked_by_user_id="user_discovery",
                    reason_code="discovery_only",
                    now=fixture.clock.now(),
                ),
            ),
    ...
    def test_revocation_authority_allows_self_and_current_workspace_owner_with_audit(
        ...
        cases = (("self", None), ("workspace_owner", "owner_001"))
        for authority, explicit_revoker in cases:
            with self.subTest(authority=authority):
                ...
                _state, _code, token = fixture.login()
                session = fixture.repository.list("oauth_token_sessions")[0]
                revoker_id = explicit_revoker or session["user_id"]
                audit_before = fixture.repository.audit_event_count
                ...
                self.assertEqual(len(audits), 1)
                audit = audits[0]
                self.assertEqual(audit["actor_user_id"], revoker_id)
    ...
    def test_revocation_authority_denies_nonowners_removed_and_disabled_without_writes(
        ...
        cases = (
            ("unrelated", "active", None, None, False, "token_revocation_forbidden"),
            (
                "member",
                "active",
                "member",
        ...
        for name, status, role, workspace_id, removed, reason_code in cases:
            with self.subTest(name=name):
                ...
                session = fixture.repository.list("oauth_token_sessions")[0]
                revoker_id = f"revoker_{name}"
                with fixture.repository.transaction() as unit:
                    fixture.repository.insert_user(
                        User(
                            user_id=revoker_id,
                            display_name=f"Revoker {name}",
                            email=f"{name}@example.test",
                            status=status,
                            created_at=fixture.clock.now_iso(),
                        )
                    )
                    if role is not None and workspace_id is not None:
                        fixture.repository.insert_workspace_member(
                            WorkspaceMember(
    ...
    def test_revocation_missing_replay_and_audit_failure_are_atomic(self) -> None:
        ...
        _state, _code, _token = replay.login()
        ...
        _state, _code, _token = audit_failure.login()
    ...
    def test_self_revocation_requires_current_workspace_membership(self) -> None:
        ...
        self.assertEqual(caught.exception.reason_code, "token_revocation_forbidden")
        fixture.repository.assert_unchanged(snapshot)
    ...
    def test_http_denial_lineage_requires_verified_token_and_server_session(self) -> None:
        ...
        revoked.bridge.revoke_token_session(
            ...
            revoked_by_user_id=revoked_session["user_id"],
            reason_code="operator_revoked",
