...
class ConnectedRuntimeLifecycleTests(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_migrate_and_bootstrap_wrappers_return_safe_results(self) -> None:
        ...
        with (
            patch.object(runtime.bridge, "revoke_token_session") as user_revoke,
            self.assertRaises(TypeError),
        ):
            runtime.revoke_token_session(
                token_session_id="oauthsid-safe",
                reason_code="operator_revoked",
                revoked_by_user_id="user_forged",  # type: ignore[call-arg]
            )
        user_revoke.assert_not_called()
        await runtime.aclose()
...
class ConnectedRuntimeCliTests(unittest.TestCase):
    ...
    def test_cli_dispatches_all_commands_and_closes_runtime(self) -> None:
        forged_runtime = _FakeCliRuntime()
        forged_error = StringIO()
        with (
            patch.object(
                ConnectedRuntimeConfig,
            ...
            patch.object(
                ConnectedRuntime,
                "compose",
                new=AsyncMock(return_value=forged_runtime),
            ) as compose,
            redirect_stderr(forged_error),
            self.assertRaises(SystemExit) as forged_exit,
        ):
            main(
                [
        ...
        self.assertEqual(forged_exit.exception.code, 2)
        compose.assert_not_awaited()
        self.assertEqual(forged_runtime.calls, [])
        self.assertEqual(forged_runtime.close_calls, 0)
