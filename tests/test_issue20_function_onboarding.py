    ...
    def test_postgres_crud_row_mapping_batch_is_manifest_onboarded(self) -> None:
        ...
        expected_category_map: dict[
            ...
            (
                ...
                "rollback_or_no_partial_state": (
                    "formowl_auth.postgres.PostgreSQLOAuthRepository."
                    "revoke_token_session does not open a transaction or commit "
                    "independently; it executes one guarded current-state update inside "
                    "the caller's existing revocation unit of work, which owns rollback "
                    "and atomicity."
                ),
                "audit_lineage": (
                    "formowl_auth.postgres.PostgreSQLOAuthRepository."
