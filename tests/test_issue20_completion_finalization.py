...
class Issue20CompletionFinalizationTests(unittest.TestCase):
    ...
    def test_non_issue20_checkbox_and_production_claim_mutations_are_rejected(self) -> None:
        source_root = self._copy_contract_root()
        source_board = (source_root / "docs/implementation-task-breakdown.md").read_text(
            encoding="utf-8"
        )
        non_issue20_checkboxes = [
            line
            for line in source_board.splitlines()
            if line.startswith(("- [ ] ", "- [x] "))
            and line != self.authority._ISSUE20_BOARD_OPEN_MARKER
        ]
        self.assertTrue(non_issue20_checkboxes)

        for checkbox_line in non_issue20_checkboxes:
            with self.subTest(checkbox_line=checkbox_line):
                root = self._copy_contract_root()
                packet = copy.deepcopy(self.packet)
                manifest = self._build_preclosure(packet, root)
                self.assertTrue(manifest)
                ...
                replacement = (
                    checkbox_line.replace("- [ ]", "- [x]", 1)
                    if checkbox_line.startswith("- [ ]")
                    else checkbox_line.replace("- [x]", "- [ ]", 1)
                )
                board_path.write_text(
                    board_text.replace(checkbox_line, replacement, 1),
                    encoding="utf-8",
                )
                self.assertEqual(self._build_transition(packet, manifest, root), {})
    ...
    def test_expected_completion_transform_preserves_durable_context(self) -> None:
        ...
        client_authority_markers = {
            ...
            "docs/agent-goals/handoff-log.md": (
                "operator-recorded predefined client ID and ChatGPT-displayed callback",
                "run the governed external campaign",
                "All seven external layers remain `not_supplied`",
            ),
            "docs/issue20-account-system-verification-status.md": (
