...
class ProductionAdapterResponseIntakeTest(unittest.TestCase):
    ...
    def test_llm_subagent_response_intake_builds_validator_accepted_candidate(self) -> None:
        ...
        response["deployment_manifest_artifact"] = _read_artifact(
            llm_packet["deployment_manifest_artifact"]
        )
        response["human_false_merge_label_artifact"] = _read_artifact(
            llm_packet["human_false_merge_label_artifact"]
        )
    ...
    def test_cli_preflight_response_validates_without_writing_artifacts(self) -> None:
        ...
        self.assertEqual(
            result["preflight_packet_type"], "production_adapter_response_preflight_v1"
        )
        ...
        self.assertEqual(result["operator_run_id"], "production_adapter_intake_run")
        self.assertEqual(result["response_packet_sha256"], intake.sha256_artifact_payload(response))
    ...
    def test_preflight_rejects_fabricated_work_packet_even_with_false_boundary(
        ...
        work_packet = {
            ...
            "canonical_packet_not_written": "inputs/production_adapter_evidence_packet.json",
            "real_evidence_root_not_written": "inputs/production_adapter_real",
            "component_collection_plan": {"plan_type": "forged"},
            "audit_collection_plan": {"plan_type": "forged"},
            "review_probe_collection_plan": {"plan_type": "forged"},
