...
class RecoveredTotalAcceptanceTest(unittest.TestCase):
    def test_recovered_total_suite_keeps_broad_missing_evidence_gates_failed(self) -> None:
        ...
        self.assertIn(
            ...
            gates["fair_external_baseline_comparison"]["blockers"],
        )
        self.assertIn(
            "human annotation results packet missing",
            gates["annotation_adjudication_protocol"]["blockers"],
        )
        self.assertIn(
            "enterprise multimodal validation packet missing",
            gates["multimodal_semantic_validation"]["blockers"],
        )
        self.assertIn(
            "production adapter evidence packet missing",
            gates["production_adapter_paths"]["blockers"],
        )

    def test_recovered_objective_audit_cannot_claim_completion(self) -> None:
        report = audit.build_report()

        self.assertFalse(report["objective_complete"])
        self.assertGreater(report["incomplete_requirement_count"], 0)
        self.assertFalse(report["claim_boundary"]["supports_goal_complete_claim"])
        self.assertIn("fair_external_baseline_comparison", report["failed_gate_ids"])
    ...
    def test_fair_external_baseline_gate_rejects_missing_human_adjudication_claim(self) -> None:
        incomplete_report = {
            ...
            "blockers": ["human answer-quality adjudication packet is not present"],
        }
        ...
        self.assertFalse(gate["passed"])
        self.assertIn("human answer-quality adjudication packet is not present", gate["blockers"])
