...
def load_input_packet() -> dict[str, Any]:
    if PACKET_PATH.is_symlink():
        return {"__input_packet_error": ("fair external baseline run packet symlink not accepted")}
    if not PACKET_PATH.exists():
        return {}
    if not PACKET_PATH.is_file():
        return {"__input_packet_error": "fair external baseline run packet is not a file"}
    if PACKET_PATH.stat().st_nlink > 1:
        return {
            "__input_packet_error": (
                "fair external baseline run packet hardlink alias not accepted"
            )
        }
