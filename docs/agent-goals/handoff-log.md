...
- User-assigned issue #44 is isolated on `uat/issue-44-orchestrator` from `5dffd68`: `/api/chat` reaches pinned `codex-cli 0.144.6` app-server threads through a private Unix socket and a narrow JSONL/WebSocket bridge; Codex decides whether to call the single FormOwl evidence tool. API-key init, non-root serving, HTTP/corpus state, and Codex state are separated; runtime attestation checks every disabled feature, and failed response/persistence paths discard divergent Codex threads. Verification passed 949 canonical tests, full Ruff/354-file format, Node 20 UI smoke, direct and Unix-socket attestation, a non-root three-container smoke, and `git diff --check`; Plato, Volta, and Mencius agreed 3/3. One authenticated live turn remains.
