...
Current local repository authority, revalidated during the 2026-07-22 scope
cleanup, is:

```text
changed / manifested / onboarded = 689 / 689 / 689
pending = 0
requested / resolved / run / pass = 509 / 509 / 509 / 509
checked evidence pairs = 1,390
...
Post-cleanup local harness and direct-trace artifacts are regenerated only
after final source freeze under
`/tmp/formowl-issue20-scope-cleanup-evidence-20260722/`. Exact SHA-256 receipts
belong in the final worker handoff rather than this contract-bound runbook.
Their seven external layers remain `not_supplied`, and their
...
preserves every non-Issue-#20 checklist line, preserves the pre-closure
verification record, records all seven layers as accepted, records reviewer
3/3 and the independent audit as passed, and does not expand any
...
checkbox, add a production-ready assertion, or apply only a subset of the five
documents. A concurrent edit or partial transition is a stop condition:
reconcile back to the exact frozen before state and restart from a new
pre-closure manifest rather than repairing hashes or continuing.
...
contract. Generic Asset governance remains outside this Issue #20 completion
claim.
