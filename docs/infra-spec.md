...
FormOwl infrastructure must support the source-preserving, graph-governed
methodology without creating source- or department-specific infrastructure
pipelines:
...
projection citations and action-proposal lineage
...
## Graph, Retrieval, Ontology, and Exact-Execution Infrastructure

PostgreSQL and pgvector remain the canonical v1 baseline. Issue #56 does not
authorize a graph-database migration, dual write, storage-engine selection
benchmark, or a parallel mail-only index.

The infrastructure supports two related paths over the same authorized
Observation snapshot.

### Knowledge-construction state

PostgreSQL stores or governs:

```text
source and Observation manifests
candidate mentions, entities, claims, relations, states, events, and frames
review decisions and entity-resolution alternatives
canonical nodes, edges, commits, and lifecycle mappings
scoped ontology core, source mappings, domain packs, and revisions
permissions, grants, policy revisions, and EffectiveGraphView metadata
source-completeness artifacts and execution fingerprints
```

Representative relational state may include:

```text
candidate_graph_objects
candidate_graph_evidence_links
graph_review_decisions
canonical_nodes
canonical_edges
...
ontology_domain_mappings
effective_graph_views
effective_graph_view_membership
graph_projection_runs
```

Exact table names may evolve through migrations, but these authorities must not
be split across ungoverned stores.

### Strong RAG projections

The minimum retrieval control is:

```text
BM25 or equivalent lexical index
+ dense Observation index in pgvector
+ deterministic fusion
+ evidence-bundle reranking
```

Lexical and dense rows bind:

```text
Observation id and source occurrence
permission and source scope
tokenizer/profile id
index revision
embedding model and revision when applicable
normalization and policy revision
content hash
```

The query and evidence sides use the same immutable tokenizer/profile. The
active target is
`jieba_sentencepiece_frozen_profile_candidate_admission_v1`. Silent fallback to
ASCII regex, substring-only retrieval, an old index, or an unpinned embedding
model is forbidden for methodology-bearing runs.

pgvector may store embeddings for authorized Observations, candidate or
canonical entities, evidence bundles, and projection sections. Vector
similarity never decides permission, canonical identity, ontology truth,
complete-set coverage, or final answer scope.

### Graph-guided query path

The runtime executes a validated `SemanticQueryPlan`:

```text
query
  -> typed router
  -> permission-filtered lexical+dense candidate admission
  -> conservative entity linking
  -> allowlisted bounded graph traversal
  -> temporal, provenance, conflict, and coverage filtering
  -> capped soft ontology bonus
  -> EvidenceBundle reranking
  -> deterministic exact executor or cited answer
```

Infrastructure must enforce frozen limits for hops, edge kinds and directions,
fan-out, candidates, evidence items, tokens, time, and repair. Every
answer-relevant path resolves to authorized Observations. Hidden or denied
nodes are not loaded and do not influence scores or counts.

Graph traversal is a read over an `EffectiveGraphView`, not direct access to all
canonical rows. Query-time entity linking, planning, traversal, and repair
create no hidden candidate, ontology, canonical graph, or permission writes.

### Ontology runtime

Ontology state is small-core, scoped, data-first, and versioned. The runtime may
use reviewed type, alias, frame, relation, source, and domain mappings for plan
validation and a capped additive retrieval score.

Hard fail-closed infrastructure checks are limited to permission, tenant and
grant scope, schema/arity, evidence lineage, revision pins, canonical-write
preconditions, exact-set coverage, and public-output redaction. An inferred
type, alias, frame, or relation mismatch receives no ontology bonus; it must not
remove otherwise admitted evidence. Historical hard ontology filtering remains
a negative ablation only.

### Deterministic exact execution

Queries asking for all, every, count, inventory, duplicate, missing item, exact
membership, aggregation, completeness, or definitive absence route to a
structured executor rather than top-k retrieval.

The executor reads bounded permission-filtered relational state and records:

```text
source and EffectiveGraphView scope
query plan and execution id
graph, ontology, policy, tokenizer, and index revisions
coverage policy and status
enumerated, policy-redacted, unsupported, and unresolved counts
duplicate policy and stable ordering
evidence lineage per returned item
```

Incomplete coverage produces a partial result and a weaker claim.

### Evidence bundles and execution fingerprints

An `EvidenceBundle` is the unit passed to reranking and the final answer model.
It records authorized Observation ids, source occurrences, evidence-backed
graph paths, score components, capped ontology contribution, conflicts,
supersession, redaction, truncation, and citations.

Every methodology-bearing execution binds one fingerprint covering:

```text
source inventory and Observation manifests
tokenizer/profile and lexical index
embedding model and dense index
graph, ontology, policy, and EffectiveGraphView revisions
router, plan schema, traversal, scorer, reranker, and exact executor
planner, extractor/linker, embedding, reranker, and answer model roles
prompt, schema, reasoning, decoding, context, evidence, token, and time budgets
evaluator, grader, code commit, dependency lock, container image, and hardware class
methodology-authority state
```

Changing a bound input creates a new fingerprint and experiment.

### Online and offline work

Source extraction, completeness reconciliation, tokenization, embedding,
candidate generation, graph/ontology projection, and large reranking
preparation run as versioned worker jobs outside MCP request handling. Online
execution performs bounded permission-filtered reads against pinned revisions.

Indexes and optional projections are rebuildable from authorized Observations
and governed graph revisions. They are not source truth and cannot bypass
PostgreSQL review, permission, provenance, or audit authority.

A dedicated graph or search engine may be considered only after a measured
scaling requirement and as a rebuildable read projection. Its introduction
requires parity, permission, lifecycle, audit, rollback, and failure-isolation
evidence; it does not reopen historical issue #33 work or replace PostgreSQL as
canonical authority.
...
revocation, and OAuth audit authority is PostgreSQL. Issue #41 must complete the
remaining generic Asset storage/authorization migration rather than treating
file-backed compatibility state as a production authority.
...
## Issue Ownership and Evidence Boundary
...
Issue #41 separately owns generic Asset tenant and owner identity binding, byte
storage, occurrence lineage, upload commit/rollback, lifecycle, retention,
purge, transfer, and cross-scope authorization. Issue #21 is a downstream mail
evidence adapter and query consumer of that generic boundary. It does not
create an alternate OAuth authority, storage plane, or connected transport.

## Deferred Infrastructure

No graph-storage selection or migration task is active. Do not add or evaluate
these systems for v1 without a measured requirement that PostgreSQL/pgvector
cannot meet:
...
Qdrant
Kafka
other dedicated graph, vector, search, or streaming platforms
```

Historical issue #33 adapters and benchmark scripts may remain for compatibility
testing, but they are not current architecture instructions. A future engine
may be introduced only as a rebuildable read projection or scaling component,
not as a replacement for PostgreSQL governance state.
...
22. Issue #20 identity infrastructure remains separate from issue #41 generic Asset governance and issue #21 downstream mail evidence consumption.
23. Strong lexical and dense retrieval use the same authorized Observation snapshot and immutable tokenizer/profile on query and evidence sides.
24. Graph traversal operates only through a permission-filtered EffectiveGraphView with frozen edge, hop, fan-out, evidence, token, and time budgets.
25. Inferred ontology compatibility is a capped additive score and cannot remove admitted evidence.
26. Exact set, count, inventory, aggregation, completeness, and definitive-negative queries use deterministic structured execution with a coverage contract.
27. Every methodology-bearing result binds its source, index, graph, ontology, permission, model, prompt, evaluator, code, image, and authority revisions into one execution fingerprint.
28. PostgreSQL/pgvector remains canonical; no historical storage benchmark or graph-database adapter is treated as an active migration plan.
```
