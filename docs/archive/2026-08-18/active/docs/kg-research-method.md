...
Methodology authority is currently valid but blocked. The semantic-query
direction below is diagnostic implementation guidance only; it does not
establish runtime alignment, methodology readiness, or methodology-quality UAT.
...
### Semantic Query Planning And Public-Term Grounding

Semantic queries are planned, not answered by row enumeration:

- The LLM is a schema- and ontology-grounded planner, not a row enumerator, and
  emits a strict typed `SemanticQueryPlan`.
- Public web search is used only for unfamiliar public terminology, and only
  after staged no-web private-safe extraction of candidate public terms and
  deterministic privacy filtering before grounding.
- MAY evidence, person names, part numbers, identifiers, and source payloads
  never leave the private boundary or enter public web search.
- Lexical mappings are data-first, versioned, and provenanced SKOS-style labels,
  not hard-coded special terms or endlessly expanding code dictionaries.
- A deterministic executor owns permission filtering, exact-set execution,
  projection, deduplication, stable ordering, count, and coverage claims.
- Ambiguous or unsafe grounding asks for clarification rather than broadening.
...
| metrics_ablations_error_analysis | passed | contract-level diagnostics only; not a real-source methodology ablation |
| methodology_runtime_alignment | blocked | required authority gate; must pass before methodology-quality claims |
