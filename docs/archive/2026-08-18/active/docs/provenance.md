...
Provenance is the invariant that connects sources, extracted observations,
candidate assertions, governed graph state, effective views, and every derived
projection or action proposal.
...
Source / Asset / EvidenceSnapshot
  ...
  -> Review Decision
  -> Canonical Commit / CanonicalGraphRevision
...
Candidate knowledge objects should record:
...
business object and assertion kind
...
projection_spec_id
graph_revision_id
user_graph_revision_id or effective_view_id when applicable
included canonical object ids when applicable
generator metadata
review state
target and backend revision reference when applicable
```

Source preservation alone is necessary but not sufficient. The core provenance
contract must also explain how observations became candidate knowledge, how
candidates became governed canonical graph state, and how that graph state
became an effective view and projection.

Wiki revisions are one projection type. Answers, reports, dashboards, review
queues, and external write proposals must preserve equivalent lineage.
