...
- Issue #20 local authority is 713/713/713 changed/manifested/onboarded,
  521/521/521/521 requested/resolved/run/pass across 1,422 checked pairs;
  direct trace is 713/713 with missing 0 and blockers `[]`, pending is 0, and
  `test_id_count` is 1,533, with zero execution or
...
- [ ] Implement issue #20 Google-backed ChatGPT MCP OAuth identity mapping and
  ...
  - Remaining state: issue #20 stays unchecked and open. Repository authority is
    713/713/713 changed/manifested/onboarded; pending, missing, extra, duplicate,
    source-binding mismatch, N/A hygiene, and validator blockers are all zero.
    Whole-manifest requested/resolved/run/pass is 521/521/521/521 across 1,422
    evidence pairs; direct trace is 713/713 with zero missing,
    `test_id_count` is 1,533, and skips, failures, errors, resolution blockers,
  ...
  - Final fixes: the implementation contract binds tracked deploy templates and
    ...
    onboarding manifest update are included in the 521-test harness authority.
  - Canonical full suite: `Ran 1533 tests in 970.250s`, `OK (skipped=9)`; Ruff check passed, 309 files are formatted, and shell/JSON/diff checks passed.
  - Latest local harness has SHA-256 `db57cd36b526b7b698ad750baeb5a4113ba4b294266efd9da79215be8b7783d2`.
...
- [ ] Align the real runtime with the active methodology authority before any further methodology-quality UAT or KG-versus-ontology claim.
  - Owner paths: issue #51 implementation, issue #52 independent acceptance, issue #54 methodology patch, `docs/methodology-authority.json`, mail/evaluator contracts, real-source evaluation, and tests.
  - Current state: authority remains blocked; on 2026-08-01 #52 recorded `RELEASE_DECISION: BLOCK` because an unprovenanced partial image reported ready while its model dependency was revoked and its retrieval path still lacked the canonical answer claim.
  - Completion proof: #51 and #54 pass on one clean bounded image; target tokenizer/Hybrid v2 and raw-source pipeline share its fingerprints; #52 then passes the complete private oracle manifest with strict authority and reviewer agreement.

## Recent Completions

- [x] Issue #36 evidence-grounded ChatGPT × FormOwl MCP evaluation completed with its documented deterministic offline claim boundary and reviewer gate.
- [x] Issue #21 governed mail evidence reading milestones completed through the tracked local deterministic checkpoints; no production-readiness claim.
- [x] Candidate KG, ontology-guided comparison, and governed effective graph query
  slices completed without canonical graph/type writes.
- [x] Remaining backbone storage, worker, folder-ingestion, and readiness-smoke slices
  completed with their historical verification evidence archived.
- [x] Completed-slice test hardening and required reviewer gates completed.

## Issues #38-#40 Maintenance Completion

- [x] Complete issues #38, #39, and #40 without weakening authority, MCP, or durable-history boundaries.
  - Proof: state-independent KG authority fixtures pass in operator and clean layouts, including a read-only repository mount for enterprise/preflight.
  - Proof: one shared MCP protocol engine and JSONL runner preserves entrypoints, fails closed on missing identity, rejects identity forgery, and requires injected semantic handlers.
  - Proof: byte-identical snapshots and SHA-256 manifest under `docs/archive/2026-07-11/`.
  - Proof: active-file retention rules, archive-integrity tests, canonical dev-container suites, and a 3/3 read-only reviewer gate pass.
