...
- The source-only r8 candidate was frozen before any oracle access. Its
  no-oracle runtime passed deterministically as `CANDIDATE_AGREE`, with the
  runner, source tree, runtime companions, packet artifacts, and no-oracle
  report bound by a separate freeze manifest.
- The subsequent exact-oracle gate rejected r8: candidate count 3, oracle count
  77, intersection 1, missing 76, and unexpected 2. The deterministic repeat
  matched, so this is a source-only candidate admission/materialization defect,
  not nondeterministic execution. r8 remains immutable rejected evidence and
  must not be patched or deployed.
- The oracle-isolated r9 implementation now has a fail-closed candidate-union
  and exact-locator bridge contract, including a same-message/different-table
  negative regression. A source-only Terra reviewer inspected all 73 coverage
  entries: 26 resolve to one direct occurrence, 23 have multiple structural
  matches, 16 carry stale or non-reproducible commitments, and 8 have no
  declared occurrence. These coverage entries mostly represent different
  unresolved sources and must not be forced onto the 89 reviewed pairs. The
  corrected r9 boundary authorizes source-reviewed pair authorities
  independently for candidate-only execution while all 73 coverage mappings
  remain source-backed non-executable and `source_complete=false`. Any genuine
  executable coverage bridge still requires exact occurrence, section, and row
  equality; message/source-family matching remains forbidden. No worker
  received oracle values, acceptance constants, browser/runtime answers, or
  the request query.
...
## Track 2 Storage-Engine Decision Boundary

PostgreSQL is the current implementation and specification baseline, not an
immutable research conclusion. On 2026-08-10 the maintainer explicitly
authorized revising `SPEC.md` and its subordinate infrastructure documents if
a controlled Track 2 comparison demonstrates that Neo4j is the better
authority store for FormOwl's governed graph workload.

Do not build two full replacement systems. Reuse one storage-neutral graph,
governance, permission, provenance, lifecycle, and query contract, then compare
only the smallest equivalent PostgreSQL and Neo4j vertical slices over the
same immutable Observation snapshot, tokenizer/profile, candidate graph,
ontology revision, actor/scope decisions, query manifest, coverage contracts,
and execution fingerprints.

The decision must measure at least:

```text
result and exact-set correctness
permission and provenance preservation
transactional review/commit/lifecycle rollback behavior
schema and ontology-revision enforcement
bounded traversal and topology diagnostics
latency, maximum RSS, rebuildability, and operational complexity
```

If Neo4j preserves every hard governance and correctness invariant and shows a
material, reproducible advantage on the graph workload, revise the canonical
specification and migrate deliberately rather than retaining PostgreSQL by
assumption. If it helps only read traversal or graph analytics, retain
PostgreSQL authority and use Neo4j only as a rebuildable projection. Do not use
dual writes as the final architecture, and do not let a storage-engine result
override the blocked tokenizer, source-completeness, or end-answer gates.
