...
1. Align runtime with the frozen Jieba + SentencePiece profile.
2. Prove source-to-Observation completeness against an oracle.
3. Implement strong hybrid RAG over the same authorized Observations.
4. Implement typed routing and deterministic exact execution.
5. Build conservative entity linking and source-backed bounded traversal.
6. Apply capped soft ontology scoring with a hard-gate negative ablation.
7. Run same-pipeline diagnostic, independent holdout, and transfer-domain
