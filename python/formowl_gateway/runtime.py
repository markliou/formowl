...
class ConnectedRuntimeConfig:
    ...
    issue56_production_semantic_enabled: bool = False
    ...
    def from_env_and_secrets(
        ...
        *,
        ...
        return cls(
            ...
            issue56_production_semantic_enabled=(
                environ.get("FORMOWL_ISSUE56_PRODUCTION_SEMANTIC_ENABLED") == "1"
            ),
        )
...
class ConnectedRuntime:
    ...
    def revoke_token_session(
        ...
        *,
        ...
        operator_service_id: str | None = None,
    ) -> dict[str, Any]:
        self._require_stateful_oauth()
        if (revoked_by_user_id is None) == (operator_service_id is None):
            raise ConnectedRuntimeError("connected_revocation_authority_invalid")
        try:
            if operator_service_id is not None:
                ...
                assert revoked_by_user_id is not None
                self.bridge.revoke_token_session(
                    token_session_id,
...
def _build_runtime_semantic_gateway(config: ConnectedRuntimeConfig) -> SemanticMcpGateway:
    ...
    if getattr(config, "issue56_production_semantic_enabled", False):
...
def _build_parser() -> argparse.ArgumentParser:
    ...
    authority.add_argument("--revoked-by-user-id")
    authority.add_argument("--operator-service-id")
    return parser
    ...
    try:
        if arguments.command == "migrate":
            ...
            payload = runtime.revoke_token_session(
                ...
                revoked_by_user_id=arguments.revoked_by_user_id,
                operator_service_id=arguments.operator_service_id,
            )
        else:
            await runtime.serve()
