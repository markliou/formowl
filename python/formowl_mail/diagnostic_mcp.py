...
from .query import StructuredSetMatch, execute_authorized_structured_set
...
_MAX_LIMIT = 20
_MAX_THIN_LEADING_HEADER_BAND_ROWS = 3
_THIN_HEADER_LABEL_COMPONENT_RE = re.compile(r"[\s/|,;:()\[\]{}\\_-]+")
...
class DiagnosticSemanticProfile:
    ...
    known_as_of: str
    _legacy_value_domain_fingerprint: bool = False
    ...
    @property
    def legacy_fingerprint_payload(self) -> dict[str, Any]:
        """Return the pre-value-domain private profile identity shape.

        This exists only to validate an already-deployed closed-enum private
        profile during a deliberate profile-only migration.  New serialization
        always uses :attr:`fingerprint_payload`, which binds value domains.
        """

        return self.legacy_fingerprint_payload_for(
            profile_id=self.profile_id,
            profile_version=self.profile_version,
            schema_alias_map=self.schema_alias_map,
            workspace_id=self.workspace_id,
        ...
        *,
        ...
        return {
            ...
            "aliases": {
                ...
                # This server-owned policy determines whether a predicate's
                # filter value is a closed ontology enum or an isolated
                # runtime-grounded public exact value.  It must be part of
                # the profile identity so a stale structural export cannot
                # silently run under a different policy.
                "value_domains": dict(schema_alias_map.value_domains),
            },
        }

    @staticmethod
    def legacy_fingerprint_payload_for(
        *,
        profile_id: str,
        profile_version: str,
        schema_alias_map: SemanticSchemaAliasMap,
        ...
        """Return the exact old closed-enum profile fingerprint payload."""
    ...
    def from_private_dict(cls, value: Mapping[str, Any]) -> "DiagnosticSemanticProfile":
        ...
        if not isinstance(aliases, Mapping) or set(aliases) not in {
            frozenset(
                {
                    "object_aliases",
                    "predicate_aliases",
        ...
        return cls(
            ...
            schema_alias_map=SemanticSchemaAliasMap(
                ...
                # Legacy profiles had only enumerated ``value_aliases``.
                # ``SemanticSchemaAliasMap`` safely derives closed_enum
                # policy for those entries.  New profiles serialize explicit
                # domains and their fingerprints therefore bind the policy.
            ...
            _legacy_value_domain_fingerprint="value_domains" not in aliases,
        )

    def to_private_dict(self) -> dict[str, Any]:
        """Serialize one server-owned profile with its value-domain policy.

        This payload is intentionally private deployment configuration; it is
        never derived from, or accepted through, the browser/MCP request
        schema.
        """

        payload = self.fingerprint_payload
        # A legacy input is accepted only for existing all-closed profiles.
        # Re-emitting it upgrades the identity to the policy-bound shape; the
        # caller must deliberately install this returned fingerprint before
        # binding it to a new deployment.  No corpus or structural row rebuild
        # is implied by this profile-only serialization step.
        payload["profile_fingerprint"] = sha256_json(payload)
        return payload
...
class _SemanticMcpRequest:
    ...
    def from_dict(cls, value: Mapping[str, Any]) -> "_SemanticMcpRequest":
        normalized = validate_semantic_request(value)
        request = cls(
            query_class=normalized["query_class"],
            object_type_mention=normalized["object_type_mention"],
            predicate_mention=normalized["predicate_mention"],
            operator=normalized["operator"],
            value_mention=normalized["value_mention"],
            projection_mention=normalized["projection_mention"],
    ...
    *,
    ...
    try:
        ...
        candidate_topology_attestation = (
            TaskAnsweringEngine._prepare_prevalidated_diagnostic_candidate_topology_attestation(
                identity_binding=template,
                structural_observations=baseline_scope.structural_observations,
            )
        )
...
def _baseline_structural_values_match_bundle(
    ...
    for observation in observations:
        ...
        source_positions.append(bundle_position_by_id[observation.source_observation_id])
...
def _template_has_runtime_topology_proof(
    ...
    if template.thin_topology_compatibility:
        return (
            ...
            and TaskAnsweringEngine._prevalidated_diagnostic_candidate_topology_attestation_is_valid(
...
def _prevalidated_pair_is_provably_unsupported(
    *,
    ...
    """Return true only when verified schema metadata excludes a canonical pair.

    The optimized canonical capability binds an exact canonical column
    ordinal. A missing canonical schema key is safe to omit from that path:
    request execution then returns ``insufficient`` without executing rows,
    never a definitive no-match. Alias-indexed rows are handled separately as
    candidate-only matches because they cannot populate that exact-column
    canonical capability. Invalid index metadata is an invariant failure, not
    an unsupported-pair signal.
    """
...
def _prevalidated_pair_has_alias_candidate_support(
    *,
    template: PrevalidatedSemanticShardTemplate,
    object_type: str,
    predicate: str,
) -> bool:
    """Recognize a declared schema alias without issuing a canonical capability.

    The canonical capability requires an exact canonical predicate column so
    that its precomputed value-column ordinal remains valid.  If the startup
    index instead proves a declared object or predicate alias, query-time
    execution may use the normal exact row executor only as candidate output.
    It must not construct a canonical claim or no-match result.
    """

    if not _prevalidated_pair_is_provably_unsupported(
        template=template,
    ...
    try:
        object_forms = frozenset(
            ...
            for value in template.profile.schema_alias_map.object_forms(object_type)
        )
        predicate_forms = frozenset(
            _normalized_semantic_text(value)
            for value in template.profile.schema_alias_map.predicate_forms(predicate)
        )
    except (
        AttributeError,
        ContractValidationError,
        ...
        raise ContractValidationError(
            "diagnostic prevalidated schema support is invalid"
        ) from error
    if not object_forms or not predicate_forms or "" in (object_forms | predicate_forms):
        raise ContractValidationError("diagnostic prevalidated schema support is invalid")

    observations = template.baseline_scope.structural_observations
    observation_count = len(observations)
    for key, candidates in template.schema_candidate_observation_ordinals.items():
        if (
        ...
        if structure_kind in object_forms and header in predicate_forms:
            return True
    return False
...
def _thin_header_label_compact(value: str) -> str:
    """Normalize one declared compact-header label without fuzzy matching."""

    return "".join(
        character for character in _normalized_semantic_text(value) if character.isalnum()
    )


def _thin_header_label_matches(
    *,
    header_label: str,
    match_forms: Sequence[str],
) -> bool:
    """Match only an exact compact-header label or delimiter-bounded component."""

    if (
        not isinstance(header_label, str)
        or not isinstance(match_forms, Sequence)
        or isinstance(match_forms, (str, bytes))
    ):
        raise ContractValidationError("diagnostic thin header label is invalid")
    try:
        declared_forms = tuple(match_forms)
    except TypeError as error:
        raise ContractValidationError("diagnostic thin header label is invalid") from error
    if not declared_forms or any(not isinstance(value, str) for value in declared_forms):
        raise ContractValidationError("diagnostic thin header label is invalid")

    normalized_header = _normalized_semantic_text(header_label)
    compact_header = _thin_header_label_compact(header_label)
    normalized_forms = frozenset(_normalized_semantic_text(value) for value in declared_forms)
    compact_forms = frozenset(_thin_header_label_compact(value) for value in declared_forms)
    if not normalized_forms or not compact_forms or "" in (normalized_forms | compact_forms):
        raise ContractValidationError("diagnostic thin header label is invalid")
    if not normalized_header or not compact_header:
        return False
    if normalized_header in normalized_forms or compact_header in compact_forms:
        return True
    components = frozenset(
        component
        for component in _THIN_HEADER_LABEL_COMPONENT_RE.split(normalized_header)
        if component
    )
    return bool(components & normalized_forms)


def _thin_leading_header_band_column_labels(
    observation: StructuralObservation,
    *,
    row_count: int,
    start_row_ordinal: int = 0,
) -> Mapping[int, str] | None:
    """Return one bounded, geometry-complete headerless row band.

    Compact exports may preserve a composite semantic table header as ordinary
    populated cells in a one- to three-row band rather than projecting
    it into ``StructuralColumn``. This candidate-only adapter accepts that
    surface only when every header row completely and unambiguously covers the
    startup-bound column ordinals. It never mutates columns, synthesizes a
    canonical schema, or permits a headerless fallback over arbitrary rows.
    """

    if not isinstance(observation, StructuralObservation):
        raise ContractValidationError("diagnostic thin header observation is invalid")
    columns = observation.columns
    rows = observation.rows
    if (
        not isinstance(row_count, int)
        or isinstance(row_count, bool)
        or not 1 <= row_count <= _MAX_THIN_LEADING_HEADER_BAND_ROWS
        or not isinstance(start_row_ordinal, int)
        or isinstance(start_row_ordinal, bool)
        or start_row_ordinal < 0
    ):
        raise ContractValidationError("diagnostic thin header band is invalid")
    if not columns or len(rows) <= start_row_ordinal + row_count:
        return None
    column_ordinals = tuple(column.column_ordinal for column in columns)
    if column_ordinals != tuple(range(len(columns))):
        raise ContractValidationError("diagnostic thin header columns are invalid")
    if any(
        _normalized_semantic_text(header)
        for column in columns
        for header in (column.original_header, column.normalized_header)
    ):
        return None
    labels: dict[int, list[str]] = {column_ordinal: [] for column_ordinal in column_ordinals}
    for relative_row_ordinal, header_row in enumerate(
        rows[start_row_ordinal : start_row_ordinal + row_count]
    ):
        expected_row_ordinal = start_row_ordinal + relative_row_ordinal
        if header_row.row_ordinal != expected_row_ordinal:
            raise ContractValidationError("diagnostic thin header row is invalid")
        coverage: set[int] = set()
        for cell in header_row.cells:
            if (
                cell.row_ordinal != expected_row_ordinal
                or cell.cell_state != "populated"
                or not isinstance(cell.value, str)
                or not _normalized_semantic_text(cell.value)
                or cell.row_span != 1
                or cell.column_span < 1
                or cell.column_ordinal < 0
                or cell.column_ordinal + cell.column_span > len(columns)
            ):
                return None
            for column_ordinal in range(
                ...
                if column_ordinal in coverage:
                    return None
                coverage.add(column_ordinal)
                labels[column_ordinal].append(cell.value)
        if coverage != set(column_ordinals):
            return None
    composites: dict[int, str] = {}
    for column_ordinal, values in labels.items():
        composite = " ".join(values)
        if not _normalized_semantic_text(composite):
            return None
        composites[column_ordinal] = composite
    return MappingProxyType(dict(sorted(composites.items())))


def _thin_leading_header_semantic_ordinals(
    observation: StructuralObservation,
    *,
    predicate_match_forms: Sequence[str],
    projection_match_forms: Sequence[str],
    start_row_ordinal: int = 0,
) -> tuple[int, frozenset[int], frozenset[int]] | None:
    """Resolve one unique candidate-only header-band binding."""

    for row_count in range(1, _MAX_THIN_LEADING_HEADER_BAND_ROWS + 1):
        labels = _thin_leading_header_band_column_labels(
            observation,
            row_count=row_count,
        ...
        if labels is None:
            continue
        predicate_ordinals = frozenset(
            column_ordinal
            for column_ordinal, header_label in labels.items()
            if _thin_header_label_matches(
                header_label=header_label,
                match_forms=predicate_match_forms,
            )
        )
        projection_ordinals = frozenset(
            column_ordinal
            for column_ordinal, header_label in labels.items()
            if _thin_header_label_matches(
                header_label=header_label,
                match_forms=projection_match_forms,
            )
        )
        if (
            len(predicate_ordinals) == 1
            and len(projection_ordinals) == 1
            and predicate_ordinals.isdisjoint(projection_ordinals)
        ):
            return row_count, predicate_ordinals, projection_ordinals
    return None


def _thin_later_header_schema_boundary_ordinal(
    observation: StructuralObservation,
    *,
    first_data_row_ordinal: int,
    predicate_match_forms: Sequence[str],
    projection_match_forms: Sequence[str],
) -> int | None:
    """Find the first later full-width row band that looks like a schema.

    A thin-topology export has no canonical header topology, so a repeated or
    changed table section cannot be safely assigned to the leading schema.
    Detect only exact alias-bearing, geometry-complete bands. Once one is
    found, the rows from that band onward are outside the leading section and
    must not contribute candidate matches. This intentionally fails closed for
    data that is indistinguishable from a later header.
    """

    if (
        not isinstance(observation, StructuralObservation)
        or not isinstance(first_data_row_ordinal, int)
        or isinstance(first_data_row_ordinal, bool)
        or first_data_row_ordinal < 0
        or isinstance(predicate_match_forms, (str, bytes))
        or isinstance(projection_match_forms, (str, bytes))
    ):
        raise ContractValidationError("diagnostic thin header boundary is invalid")
    rows = observation.rows
    if first_data_row_ordinal >= len(rows):
        return None
    for start_row_ordinal in range(first_data_row_ordinal, len(rows)):
        for row_count in range(1, _MAX_THIN_LEADING_HEADER_BAND_ROWS + 1):
            labels = _thin_leading_header_band_column_labels(
                observation,
                row_count=row_count,
            ...
            if predicate_columns and projection_columns:
                return start_row_ordinal
    return None


def _thin_candidate_row_matches(
    *,
    row: Any,
    predicate_ordinals: frozenset[int],
    value_match_forms: Sequence[str],
) -> bool:
    """Apply the generic executor's exact value equality on bound columns."""

    if isinstance(value_match_forms, (str, bytes)):
        raise ContractValidationError("diagnostic thin row value forms are invalid")
    normalized_values = frozenset(
        _normalized_semantic_text(value) for value in value_match_forms if isinstance(value, str)
    )
    ...
    return any(
        ...
        and any(
            _normalized_semantic_text(value) in normalized_values
...
def _thin_candidate_projection_values(
    *,
    row: Any,
    projection_ordinals: frozenset[int],
) -> tuple[str, ...]:
    """Project only populated cells from the uniquely bound header column."""

    values: list[str] = []
    for cell in row.cells:
        if (
            cell.cell_state != "populated"
            or cell.column_ordinal not in projection_ordinals
            or not isinstance(cell.value, str)
            or not cell.value
...
def _execute_thin_leading_header_candidates(
    *,
    template: PrevalidatedSemanticShardTemplate,
    plan: Any,
) -> tuple[StructuredSetMatch, ...]:
    """Recover bounded candidate rows from the compact leading-header shape.

    The helper is intentionally private to the thin-topology compatibility
    branch. It reads only the exact startup-bound observation tuple, requires
    one unambiguous leading-row mapping, and returns candidate matches only.
    No coverage conclusion, canonical claim, or no-match conclusion is
    produced here.
    """

    if (
        not isinstance(template, PrevalidatedSemanticShardTemplate)
        or not template.thin_topology_compatibility
        or not _template_has_runtime_topology_proof(template)
        or isinstance(template.structural_relevant_item_ids, (str, bytes))
    ):
        raise ContractValidationError("diagnostic thin header template is invalid")
    try:
        object_forms = frozenset(
            _normalized_semantic_text(value) for value in tuple(plan.object_type_match_forms)
        )
        predicate_forms = tuple(plan.predicate_match_forms)
        value_forms = tuple(plan.value_match_forms)
        projection_forms = tuple(plan.projection_match_forms)
    except (AttributeError, TypeError, ValueError) as error:
        raise ContractValidationError("diagnostic thin header plan is invalid") from error
    if (
        not object_forms
        or "" in object_forms
        or not predicate_forms
        ...
        raise ContractValidationError("diagnostic thin header plan is invalid")
    authorized_item_ids = frozenset(template.structural_relevant_item_ids)
    if not authorized_item_ids or any(
        not isinstance(item_id, str) or not item_id for item_id in authorized_item_ids
    ):
        raise ContractValidationError("diagnostic thin header scope is invalid")

    matches: list[StructuredSetMatch] = []
    for observation in template.baseline_scope.structural_observations:
        if observation.source_inventory_item_id not in authorized_item_ids:
            raise ContractValidationError("diagnostic thin header scope is invalid")
        if _normalized_semantic_text(observation.structure_kind) not in object_forms:
            continue
        resolved_ordinals = _thin_leading_header_semantic_ordinals(
            observation,
            predicate_match_forms=predicate_forms,
            projection_match_forms=projection_forms,
        )
        if resolved_ordinals is None:
            continue
        header_row_count, predicate_ordinals, projection_ordinals = resolved_ordinals
        boundary_ordinal = _thin_later_header_schema_boundary_ordinal(
            observation,
            first_data_row_ordinal=header_row_count,
            predicate_match_forms=predicate_forms,
            projection_match_forms=projection_forms,
        )
        rows = observation.rows[
            header_row_count : (
                boundary_ordinal if boundary_ordinal is not None else len(observation.rows)
            )
        ]
        for row in rows:
            if not _thin_candidate_row_matches(
                row=row,
                predicate_ordinals=predicate_ordinals,
                value_match_forms=value_forms,
            ):
                continue
            matches.append(
                StructuredSetMatch(
                    ...
                    projection_values=_thin_candidate_projection_values(
                        row=row,
...
def _execute_prevalidated_sharded_semantic_request(
    *,
    ...
    serialized_bytes = 2
    found_in_any_complete_shard = False
    candidate_only_aggregate = False
    ...
    try:
        for template in templates:
            ...
            if isinstance(execution_scope, _PrevalidatedDiagnosticExecutionScope):
                ...
                matches = (
                    *execution.matches,
                    *_execute_thin_leading_header_candidates(
                        template=template,
                        plan=plan,
                    ),
                )
                candidate_only_aggregate = True
            elif _prevalidated_pair_has_alias_candidate_support(
                template=template,
                object_type=plan.object_type,
                predicate=plan.predicate,
            ):
                # The exact canonical capability binds a canonical predicate
                # column ordinal. A declared alias can be executed safely by
                # the normal row matcher, but only as candidate output: never
                # as a canonical answer or aggregate no-match claim.
                selected_observations = _select_prevalidated_schema_candidates(
                    template,
                    plan=plan,
                )
                ...
                matches = execution.matches
                candidate_only_aggregate = True
            elif _prevalidated_pair_is_provably_unsupported(
                template=template,
                object_type=plan.object_type,
                predicate=plan.predicate,
            ):
                # A startup-bound strict shard can prove this schema pair is
                # unavailable without reading rows.  It cannot establish a
                # complete aggregate no-match claim, so retain candidate-only
                # semantics while scanning later independently bound shards.
                candidate_only_aggregate = True
                continue
            else:
                return _semantic_insufficient_payload(request)
    ...
    if candidate_only_aggregate and not matched_in_any_shard:
        return _semantic_insufficient_payload(request)
    ...
    return {
        ...
        "claim_state": (
            "CANDIDATE_MATCHES"
            if candidate_only_aggregate
            else ("FOUND" if found_in_any_complete_shard else "NOT_FOUND_WITHIN_COMPLETE_SCOPE")
        ),
        "complete_projection": complete_projection,
        "display_pagination": {
            "page_size": request.page_size,
...
def _validate_tool_arguments(arguments: Mapping[str, Any]) -> None:
    ...
    semantic_request = arguments.get("semantic_request")
    if semantic_request is not None:
        if not isinstance(semantic_request, Mapping):
            raise ContractValidationError("MCP semantic request is invalid")
        validate_semantic_request(semantic_request)
