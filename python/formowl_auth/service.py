...
class FormOwlOAuthBridge:
    ...
    def revoke_token_session(
        ...
        *,
        ...
        _safe_id(revoked_by_user_id, "revoked_by_user_id")
        _safe_id(reason_code, "reason_code")
        with self.repository.transaction() as unit:
            ...
            revoker = self._require_active_user(revoked_by_user_id)
            revoker_membership = self.repository.get_active_workspace_member(
                revoker.user_id,
                session.current_workspace_id,
            )
            if revoker_membership is None or (
                revoker.user_id != session.user_id and revoker_membership.role != "owner"
            ):
                raise OAuthAccessDenied(
                    "access_denied",
            ...
            self.repository.revoke_token_session(
                token_session_id,
                revoked_at=_iso(now),
                reason_code=reason_code,
            )
